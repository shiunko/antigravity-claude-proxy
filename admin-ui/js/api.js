...
class ApiClient {
  ...
  async login(username, password) {
    const result = await this.request("POST", "/admin/auth/login", {
      username,
      password,
    });
    if (result.token) {
  ...
  async changePassword(currentPassword, newPassword) {
    return this.request("POST", "/admin/auth/change-password", {
      currentPassword,
      newPassword,
    });
  }
  ...
  async removeModelFromGroup(groupId, modelName) {
    return this.request(
      "DELETE",
      `/admin/groups/${groupId}/models/${encodeURIComponent(modelName)}`,
    );
  }
