...
export class Orchestrator {
    ...
    async handle(request, context) {
        ...
        const modelCandidates = this.modelAggregator.resolve(userId, requestedModel);

        console.log(`[Orchestrator] User ${userId} requested '${requestedModel}', resolved to:`, modelCandidates);
        ...
        for (const model of modelCandidates) {
            try {
                ...
                const isAuthError = error.message.includes('Auth') || error.status === 401; // Simple check
