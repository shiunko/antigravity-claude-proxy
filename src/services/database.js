...
export function addAccount(accountData) {
  ...
  const stmt = db.prepare(
    `INSERT INTO upstream_accounts (${columns}) VALUES (${placeholders})`,
  );
...
export function updateAccount(id, updates) {
  ...
  const stmt = db.prepare(
    `UPDATE upstream_accounts SET ${setClause} WHERE id = ?`,
  );
  return stmt.run(...values);
}
...
export function createModelGroup(userId, alias, strategy = "priority") {
  const db = getDb();
  const stmt = db.prepare(
    "INSERT INTO model_groups (user_id, alias, strategy) VALUES (?, ?, ?)",
  );
...
export function addModelToGroup(groupId, modelName, orderIndex = 0) {
  const db = getDb();
  const stmt = db.prepare(
    "INSERT INTO model_group_items (group_id, model_name, order_index) VALUES (?, ?, ?)",
  );
...
export function getModelGroup(userId, alias) {
  ...
  const groupStmt = db.prepare(
    "SELECT * FROM model_groups WHERE user_id = ? AND alias = ?",
  );
  ...
  const itemsStmt = db.prepare(
    "SELECT * FROM model_group_items WHERE group_id = ? ORDER BY order_index ASC",
  );
...
export function removeModelFromGroup(groupId, modelName) {
  const db = getDb();
  const stmt = db.prepare(
    "DELETE FROM model_group_items WHERE group_id = ? AND model_name = ?",
  );
